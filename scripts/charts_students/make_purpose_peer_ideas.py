# Usage:
#   python3 -m scripts.charts_students.make_purpose_peer_ideas
...
OUT_DIR    = Path("figs/students/purpose_peer_ideas")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ---- Config -----------------------------------------------------------------
MIN_RESP = 2
TITLE_SCHOOL = "目的：クラスの人の考えを参考にするため（0–3）— 学校別（学生）"
TITLE_GRADE  = "目的：クラスの人の考えを参考にするため（0–3）— 学年別（学生）"
OUT_SCHOOL_PNG = OUT_DIR / "purpose_peer_ideas_by_school.png"
OUT_SCHOOL_CSV = OUT_DIR / "purpose_peer_ideas_by_school.csv"
OUT_GRADE_PNG  = OUT_DIR / "purpose_peer_ideas_by_grade.png"
OUT_GRADE_CSV  = OUT_DIR / "purpose_peer_ideas_by_grade.csv"
...
CAND_COLS = [
    "クラスの人の考えを参考にするため",
    # tolerate minor spacing variants if any exist
    "クラスの人の考えを 参考にするため",
]
...
def find_question_col(df: pd.DataFrame) -> str | None:
    ...
    for c in df.columns:
        nc = _norm(c)
        if ("クラス" in nc) and ("考え" in nc) and ("参考" in nc):
            return c
    return None
...
def find_grade_col(df: pd.DataFrame) -> str | None:
    ...
    for cand in ["あなたの学年を教えてください", "学年", "学年（整理前）"]:
        if cand in df.columns:
            return cand
    # last resort: anything containing 学年
    for c in df.columns:
        if "学年" in str(c):
            return c
    return None
...
SCALE_MAP = {
    ...
    "使用することがある": 2,
    "頻繁（ひんぱん）に使用している": 3,
    # tolerate slight variants (without ruby)
    "頻繁に使用している": 3,
}
...
def main():
    ...
    if not col_q:
        raise KeyError("質問列（クラスの人の考えを参考にするため）が見つかりません。")
    ...
    # order by your canonical allowed list if available; otherwise keep computed order
    ...
    # ------- by GRADE -------
    # Basic cleaning of grade labels (use your canon if present)
    # We assume '学年_canon' is already standardized into values like:
    # 小1..小6 / 中1..中3 / 高1..高3 など
    ...
    # Optional: if you have a canonical grade order column in your cleaner, sort by it.
    # Otherwise, sort naturally by avg desc for readability.
    g_grade = g_grade.sort_values("avg_score", ascending=False)
