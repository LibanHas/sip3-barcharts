# Usage:
#   python3 -m scripts.charts_students.make_purpose_peer_habits
...
OUT_DIR    = Path("figs/students/purpose_peer_habits")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ---- Config -----------------------------------------------------------------
MIN_RESP = 2
TITLE_SCHOOL = "目的：クラスの人の学び方を参考にするため（0–3）— 学校別（学生）"
TITLE_GRADE  = "目的：クラスの人の学び方を参考にするため（0–3）— 学年別（学生）"
OUT_SCHOOL_PNG = OUT_DIR / "purpose_peer_habits_by_school.png"
OUT_SCHOOL_CSV = OUT_DIR / "purpose_peer_habits_by_school.csv"
OUT_GRADE_PNG  = OUT_DIR / "purpose_peer_habits_by_grade.png"
OUT_GRADE_CSV  = OUT_DIR / "purpose_peer_habits_by_grade.csv"
...
CAND_COLS = [
    "クラスの人の学び方を参考にするため",
    "クラスの人の学び方を 参考にするため",  # tolerate accidental spacing
]
...
def find_question_col(df: pd.DataFrame) -> str | None:
    ...
    for c in df.columns:
        nc = _norm(c)
        if ("クラス" in nc) and ("学び方" in nc) and ("参考" in nc):
            return c
    return None
...
def main():
    ...
    if not col_q:
        raise KeyError("質問列（クラスの人の学び方を参考にするため）が見つかりません。")
